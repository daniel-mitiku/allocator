...
export const availabilityTemplateSchema = z
  ...
  .refine(
    (data) => {
      ...
      return data.availabilityBlocks.every(
        (slot) => slot.endTime > slot.startTime
      );
    },
    {
      message: "End time must be after start time for all time blocks.",
      path: ["availabilityBlocks"], // You can specify a path to show the error
    }
  );
...
export const createScheduleInstanceSchema = z.object({
  ...
  availabilityTemplateId: z
    .string()
    .min(1, "An availability template is required for the schedule."),
