...
import {
  ...
  type scheduleInstanceStatusSchema,
  getEventsAtTimeSlotSchema,
  getAvailableResourcesSchema,
  updateScheduledEventResourceSchema,
} from "./schemas"
import { AttendeeLevel, DayOfWeek, type ScheduleInstance } from "@prisma/client"
 ...
 */
export async function createCourse(input: z.infer<typeof createCourseSchema>): Promise<ActionResponse<any>> {
 ...
 */
export async function updateCourse(input: z.infer<typeof updateCourseSchema>): Promise<ActionResponse<any>> {
 ...
 */
export async function deleteCourse(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
    ...
    return {
      success: false,
      message: "Failed to delete course. It might be assigned to a schedule or another related entity.",
    }
  }
}
 ...
 */
export async function createUser(input: z.infer<typeof userSchema>): Promise<ActionResponse<any>> {
 ...
 */
export async function updateUser(input: z.infer<typeof updateUserSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a User.
 */
export async function deleteUser(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 * Creates a new Room.
 */
export async function createRoom(input: z.infer<typeof roomSchema>): Promise<ActionResponse<any>> {
 ...
 * Updates a Room.
 */
export async function updateRoom(input: z.infer<typeof updateRoomSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a Room.
 */
export async function deleteRoom(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 */
export async function bulkCreateRooms(input: z.infer<typeof roomSchema>[]): Promise<ActionResponse<any>> {
  ...
  try {
    ...
    const roomsToCreate = validation.data.filter((room) => !existingNames.has(room.name))
 ...
 */
export async function createProgram(input: z.infer<typeof programSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a Program.
 */
export async function deleteProgram(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 * Creates a new Batch.
 */
export async function createBatch(input: z.infer<typeof batchSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a Batch.
 */
export async function deleteBatch(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 */
export async function createSection(input: z.infer<typeof sectionSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a Section.
 */
export async function deleteSection(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 * Creates a new Group.
 */
export async function createGroup(input: z.infer<typeof groupSchema>): Promise<ActionResponse<any>> {
 ...
 * Deletes a Group.
 */
export async function deleteGroup(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 */
export async function bulkCreateProgram(input: z.infer<typeof bulkProgramSchema>): Promise<ActionResponse<any>> {
  ...
  try {
    ...
    return {
      success: false,
      message: "An unexpected error occurred. Failed to create the program structure.",
    }
  }
}
 ...
 */
export async function bulkCreateGroups(input: z.infer<typeof bulkCreateGroupsSchema>): Promise<ActionResponse<any>> {
...
export async function createSectionWithGroups(
  ...
  try {
    ...
    return {
      success: true,
      message: `Section '${name}' and ${groups?.length || 0} groups created successfully.`,
    }
  } catch (e) {
 ...
 */
export async function deleteAvailabilityTemplate(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
 ...
 */
export async function getAllScheduleInstances(): Promise<ActionResponse<ScheduleInstance[]>> {
  try {
...
export async function getScheduleInstanceDetails(id: string) {
  const [scheduleInstance, allCourses, allPersonnel, allRooms, allSections] = await Promise.all([
...
export async function updateScheduleSolverSettings(
  ...
  const { scheduleInstanceId, roomStickinessWeight, spacingPreference } = validation.data
 ...
 */
export async function deleteScheduleInstance(input: z.infer<typeof idSchema>): Promise<ActionResponse<any>> {
  try {
...
export async function assignResourcesToSchedule(
  ...
  const { scheduleInstanceId, courseIds, sectionIds, personnelIds, roomIds } = validation.data
 ...
 */
export async function submitPreferences(input: z.infer<typeof preferencesSchema>): Promise<ActionResponse<any>> {
...
async function getScheduleDataForSolver(scheduleInstanceId: string) {
  ...
  const historicalPreferences = new Map<string, { count: number; avgPerformance: number }>()

  historicalAssignments.forEach((assignment) => {
    const key = `${assignment.personnelId}-${assignment.courseId}`
    const existing = historicalPreferences.get(key) || { count: 0, avgPerformance: 0 }

    // Simple performance scoring (could be enhanced)
  ...
  // ... existing activities processing code ...
  ...
  const personnel = schedule.personnel.map((person) => ({
    id: person.id,
    name: person.name,
    roles: person.roles,
    historical_courses: schedule.courses
      .map((course) => {
        const key = `${person.id}-${course.id}`
        const historical = historicalPreferences.get(key)
        return {
          courseId: course.id,
          experience_count: historical?.count || 0,
          performance_score: historical?.avgPerformance || 0,
          preference_weight: historical ? Math.min(historical.count * 0.2 + historical.avgPerformance * 0.1, 1.0) : 0,
        }
      })
      .filter((course) => course.experience_count > 0 || course.preference_weight > 0),
  }))

  // ... existing rooms and preferences processing code ...
  ...
  const preferences = schedule.preferences.map((pref) => ({
    personnel_id: pref.personnelId,
    activity_id: pref.activityTemplateId,
    rank: pref.rank,
  }))

  const time_preferences = schedule.timePreferences.map((pref) => ({
    time_slot: timeToSlotIndex(pref.time),
    preference_score: pref.rank,
  }))
  ...
  return {
    ...
    preferences,
    time_preferences,
    historical_preferences: Array.from(historicalPreferences.entries()).map(([key, value]) => {
      const [personnelId, courseId] = key.split("-")
      return {
        personnel_id: personnelId,
        course_id: courseId,
        experience_count: value.count,
        performance_score: value.avgPerformance,
      }
    }),
    solver_parameters: {
      room_stickiness_weight: schedule.roomStickinessWeight || 1.0,
      spacing_preference: schedule.spacingPreference || "NONE",
      historical_weight: 0.3, // Weight for historical preferences in allocation
    },
  }
}
...
async function saveSolutionToDatabase(scheduleInstanceId: string, solution: any[]) {
  ...
  // Get schedule info for historical records
  const schedule = await prisma.scheduleInstance.findUnique({
    where: { id: scheduleInstanceId },
    include: {
      courses: { include: { activityTemplates: true } },
  ...
  const eventsToCreate = solution.map((event) => {
    ...
    return {
      ...
      endTime: endTime,
      scheduleInstanceId: scheduleInstanceId,
      activityTemplateId: event.templateId,
  ...
  const historicalRecords = solution.flatMap((event) => {
    const template = schedule?.courses.flatMap((c) => c.activityTemplates).find((t) => t.id === event.templateId)

    if (!template) return []

    return event.personnel_ids.map((personnelId: string) => ({
      personnelId,
      ...
      collegeId: schedule?.collegeId,
      assignedAt: new Date(),
      performance: null, // Will be updated later based on feedback
    }))
  })
 ...
 */
export async function triggerAllocation(scheduleInstanceId: string): Promise<ActionResponse<any>> {
  ...
  try {
    ...
    if (solverInput.activities.length === 0) {
      return {
        success: false,
        message: "No activities to schedule. Please assign courses and sections.",
      }
    }
    if (solverInput.time_slots.length === 0) {
      return {
        success: false,
        message: "No available time slots defined. Please set an availability template.",
      }
    }
    ...
    return {
      success: true,
      message: "Allocation completed successfully! The timetable is now available.",
    }
  } catch (error: any) {
 ...
 */
export async function exportAllocationData(scheduleInstanceId: string): Promise<ActionResponse<any>> {
  try {
    const solverInput = await getScheduleDataForSolver(scheduleInstanceId)
    return {
      success: true,
...
): Promise<ActionResponse<{ availablePersonnel: any[]; availableRooms: any[] }>> {
