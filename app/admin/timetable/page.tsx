...
import type { ScheduleInstance, DayOfWeek } from "@prisma/client"
...
import {
  type FullScheduleInstance,
  type FullScheduledEvent,
  type TimetableStatusOverview,
  type ActiveTimeSlot,
  type AvailablePersonnel,
  type AvailableRoom,
  timeToSlotIndex,
  slotToTime,
  generateTimeSlots,
...
import { ManualAllocationSidebar } from "./_components/manual-allocation-sidebar"
...
export default function TimetableManagementPage() {
  const [allScheduleInstances, setAllScheduleInstances] = useState<ScheduleInstance[]>([])
  const [selectedScheduleInstanceId, setSelectedScheduleInstanceId] = useState<string | null>(null)
  const [scheduleInstanceDetails, setScheduleInstanceDetails] = useState<FullScheduleInstance | null>(null)
  const [overviewStatuses, setOverviewStatuses] = useState<TimetableStatusOverview[]>([])

  const [activeTimeSlot, setActiveTimeSlot] = useState<ActiveTimeSlot>(null)
  const [eventsAtTimeSlot, setEventsAtTimeSlot] = useState<FullScheduledEvent[]>([])
  const [freeResourcesAtTimeSlot, setFreeResourcesAtTimeSlot] = useState<{
    availablePersonnel: AvailablePersonnel[]
    availableRooms: AvailableRoom[]
  }>({ availablePersonnel: [], availableRooms: [] })
  const [selectedEventToAssign, setSelectedEventToAssign] = useState<FullScheduledEvent | null>(null)
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (statusesResult.success && statusesResult.data) {
        ...
        toast.error(statusesResult.message || "Failed to load schedule statuses.")
      }
  ...
  useEffect(() => {
    const loadScheduleDetails = async () => {
      ...
      if (result.success && result.data) {
        setScheduleInstanceDetails(result.data.scheduleInstance as FullScheduleInstance)
      } else {
  ...
  const loadSidebarData = useCallback(async () => {
    ...
    if (eventsResult.success && eventsResult.data) {
      ...
      toast.error(eventsResult.message || "Failed to load events for time slot.")
      setEventsAtTimeSlot([])
    }
    ...
    if (freeResourcesResult.success && freeResourcesResult.data) {
      ...
      toast.error(freeResourcesResult.message || "Failed to load free resources.")
      setFreeResourcesAtTimeSlot({
        availablePersonnel: [],
        availableRooms: [],
      })
  ...
  const allDisplayTimeSlots = useMemo(() => generateTimeSlots(MIN_HOUR, MAX_HOUR), [])
  ...
  const getEventInfoForSlot = useCallback(
    (day: DayOfWeek, startTime: string) => {
      if (!scheduleInstanceDetails) return { hasEvent: false, eventEndTime: null }
  ...
  const handleScheduledEventUpdate = useCallback(
    (updatedEvent: FullScheduledEvent) => {
      ...
      setEventsAtTimeSlot((prev) => prev.map((event) => (event.id === updatedEvent.id ? updatedEvent : event)))
      ...
      setScheduleInstanceDetails((prev) => {
        if (!prev) return null
        const updatedEvents = prev.scheduledEvents.map((e) => (e.id === updatedEvent.id ? updatedEvent : e))
        return { ...prev, scheduledEvents: updatedEvents }
      })
      // If the event being updated was the one selected for assignment, update it
      if (selectedEventToAssign && selectedEventToAssign.id === updatedEvent.id) {
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-4xl font-extrabold mb-8 text-center ">Global Timetable Management</h1>
        ...
        <CardContent className="p-0 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 w-full md:w-auto">
            ...
            >
              ...
              <SelectContent className="max-h-60 overflow-y-auto">
                {allScheduleInstances.length === 0 && <div className="p-2 text-sm ">No schedules available.</div>}
                {allScheduleInstances.map((instance) => (
                  <SelectItem key={instance.id} value={instance.id}>
                    {instance.name} ({instance.startDate.toLocaleDateString()} - {instance.endDate.toLocaleDateString()}
                    )
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="flex-1 w-full md:w-auto grid grid-cols-1 sm:grid-cols-3 gap-4">
            {overviewStatuses.map((status) => (
              <Card key={status.id} className="p-3 shadow-md rounded-lg text-center ">
                <p className="text-sm font-semibold truncate">{status.name}</p>
                <div className="mt-1">
                  {status.status === "COMPLETED" && (
                    <Badge className="text-green-700 px-3 py-1 rounded-full">Completed</Badge>
                  )}
                  {status.status === "SEMI_ALLOCATED" && (
                    <Badge className="text-yellow-700 px-3 py-1 rounded-full">
                      Semi-Allocated ({status.currentlyScheduledEvents}/{status.totalActivitiesToSchedule})
                    </Badge>
                  )}
                  {status.status === "NOT_SCHEDULED" && (
                    <Badge variant="destructive" className="text-red-700 px-3 py-1 rounded-full">
                      Not Scheduled
                    </Badge>
                  )}
                </div>
      ...
      {isLoadingDetails ? (
        ...
        <p className="text-center text-xl text-muted py-20">Please select a schedule instance to view its timetable.</p>
      ) : (
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <Card className="flex-1 min-w-0 shadow-lg rounded-xl overflow-hidden">
            <CardHeader className="p-4 bg-muted border-b">
              ...
              <p className="text-sm text-muted-foreground">
                Click on a time block to view/edit scheduled events and assign free resources.
              </p>
            </CardHeader>
                  ...
                  <tbody className="divide-y divide-muted">
                    {orderedDays.map((day) => (
                      <tr key={day}>
                        ...
                        {allDisplayTimeSlots.map((slot) => {
                          const availabilityBlock = scheduleInstanceDetails.availabilityTemplate.availableSlots.find(
                            (block) =>
                              block.dayOfWeek === day &&
                              timeToSlotIndex(slot.value) >= timeToSlotIndex(block.startTime) &&
                              timeToSlotIndex(slot.value) < timeToSlotIndex(block.endTime),
                          )
                          const isAvailableTime = !!availabilityBlock
                          const { hasEvent, eventEndTime } = getEventInfoForSlot(day, slot.value)

                          let cellClasses =
                            "h-12 px-2 py-1 border border-muted cursor-pointer text-center text-xs relative overflow-hidden"
                          if (!isAvailableTime) {
                            cellClasses += " bg-muted text-muted opacity-60 pointer-events-none"
                          } else if (hasEvent) {
                            cellClasses += " bg-indigo-100 hover:bg-indigo-200 text-indigo-800 font-semibold"
                          } else {
                            cellClasses += "  hover:bg-muted text-muted"
                          }

                          const isActiveSlot =
                            activeTimeSlot?.dayOfWeek === day && activeTimeSlot.startTime === slot.value
                          if (isActiveSlot) {
                          ...
                          const durationSlots = eventEndTime
                            ? timeToSlotIndex(eventEndTime) - timeToSlotIndex(slot.value)
                            : 0
                          const colSpan = hasEvent ? Math.max(1, durationSlots) : 1
                            ...
                            <td
                              ...
                              onClick={() =>
                                setActiveTimeSlot({
                                  ...
                                  endTime: slotToTime(timeToSlotIndex(slot.value) + 1), // 30 mins later
                                })
                              }
                            >
                              {isAvailableTime &&
                                  ...
                                  isStartingEvent && (
                                    <div className="absolute inset-0 flex items-center justify-center p-1 text-xs leading-none">
                                      {scheduleInstanceDetails.scheduledEvents
                                        .filter((e) => e.dayOfWeek === day && e.startTime === slot.value)
                                        .map((e) =>
          ...
          {/* Right Section: Enhanced Manual Allocation Sidebar */}
          <div className="lg:w-1/3">
            <ManualAllocationSidebar
              activeTimeSlot={activeTimeSlot}
              eventsAtTimeSlot={eventsAtTimeSlot}
              freeResourcesAtTimeSlot={freeResourcesAtTimeSlot}
