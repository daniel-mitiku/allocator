...
export async function bulkCreateProgram(
  ...
  const { name, collegeId, batches } = validation.data;
  ...
  try {
    ...
    if (existingProgram) {
      return {
        success: false,
        message: `A program named '${name}' already exists in this college.`,
      };
    }
...
async function getScheduleDataForSolver(scheduleInstanceId: string) {
  ...
  const historicalPreferences = new Map<
    string,
    { count: number; avgPerformance: number }
  >();

  historicalAssignments.forEach((assignment) => {
    const key = `${assignment.personnelId}-${assignment.courseId}`;
    const existing = historicalPreferences.get(key) || {
      count: 0,
      avgPerformance: 0,
    };
    ...
    historicalPreferences.set(key, {
      ...
      avgPerformance:
        (existing.avgPerformance * existing.count + performanceScore) /
        (existing.count + 1),
    });
  });
  ...
  dayOrder.forEach((day, dayIndex) => {
    const blocksForDay =
      schedule.availabilityTemplate.availabilityBlocks.filter(
        (s) => s.dayOfWeek === day
      );
    ...
    historical_courses: schedule.courses
      .map((course) => {
        ...
        return {
          ...
          preference_weight: historical
            ? Math.min(
                historical.count * 0.2 + historical.avgPerformance * 0.1,
                1.0
              )
            : 0,
        };
      ...
      .filter(
        (course) => course.experience_count > 0 || course.preference_weight > 0
      ),
  }));
  ...
  return {
    ...
    preferences,
    time_preferences,
    historical_preferences: Array.from(historicalPreferences.entries()).map(
      ([key, value]) => {
...
async function saveSolutionToDatabase(
  ...
  const historicalRecords = solution.flatMap((event) => {
    const template = schedule?.courses
      .flatMap((c) => c.activityTemplates)
      .find((t) => t.id === event.templateId);
...
export async function updateScheduledEventResources(
  ...
  const { scheduledEventId, roomIds, personnelIds } = validation.data;
